...
function PlatformMatchQuizPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/api/platform-match/start")
      .then(res => {
        if (!res.ok) throw new Error("HTTP error " + res.status);
        return res.json();
      })
      .then(data => {
        console.log("Dane quizu Platform Match:", data);
        setQuestions(data);
      })
      .catch(err => console.error("Błąd API Platform Match:", err));
  }, []);
  ...
  useEffect(() => {
    if (finished) {
      const saveResult = async () => {
        ...
        try {
          const res = await fetch("http://localhost:8080/api/platform-match/save-result", {
            method: "POST",
          ...
          console.log("Wynik Platform Match zapisany:", responseText);
        } catch (err) {
          console.error("Błąd zapisu wyniku Platform Match:", err);
        }
      };
